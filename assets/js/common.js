...
function toggleTheme() {
    ...
    if (bodyEl.classList.contains('light-theme')) {
        ...
        navbarEl.classList.remove('navbar-light');
        ...
        for (let i = 0; i < pubs.length; i++) {
            pubs[i].classList.add('bg-dark');
            pubs[i].classList.remove('bg-white');
        }
        ...
        for (let i = 0; i < pubs.length; i++) {
            pubs[i].classList.add('bg-white');
            pubs[i].classList.remove('bg-dark');
        }
        buttonEl.innerText = '🌙';
